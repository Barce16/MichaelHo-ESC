<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800">Package Usage Report</h2>
        </div>
    </x-slot>
            ...
            <div class="bg-white rounded-lg shadow-sm p-8">
                ...
                <div class="border-b-2 border-gray-300 pb-6 mb-6">
                    <div class="flex items-start justify-between">
                        ...
                        <div class="text-right">
                            <h2 class="text-xl font-bold text-gray-900">Package Usage Report</h2>
                ...
                <div class="grid md:grid-cols-4 gap-6 mb-8">
                    <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg p-6 border-2 border-orange-200">
                        <div class="text-sm text-orange-700 font-medium mb-1">Total Packages</div>
                        <div class="text-4xl font-bold text-orange-900">{{ $stats['total_packages'] }}</div>
                    </div>

                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-6 border-2 border-blue-200">
                        <div class="text-sm text-blue-700 font-medium mb-1">Total Bookings</div>
                        <div class="text-4xl font-bold text-blue-900">{{ $stats['total_bookings'] }}</div>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-6 border-2 border-purple-200">
                        <div class="text-sm text-purple-700 font-medium mb-1">Most Popular</div>
                        <div class="text-lg font-bold text-purple-900 truncate">{{ $stats['most_popular']->name }}</div>
                        <div class="text-sm text-purple-700">{{ $stats['most_popular']->total_events }} bookings</div>
                    </div>
                    @endif
                </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b-2 border-gray-300">
                                <th class="text-left py-3 font-semibold text-gray-700">Rank</th>
                                <th class="text-left py-3 font-semibold text-gray-700">Package Name</th>
                                <th class="text-left py-3 font-semibold text-gray-700">Type</th>
                                <th class="text-right py-3 font-semibold text-gray-700">Package Price</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-gray-200">
                                <td class="py-3 font-bold text-gray-900">{{ $index + 1 }}</td>
                                <td class="py-3 font-medium">{{ $package->name }}</td>
                                <td class="py-3">{{ $package->type ?? '-' }}</td>
                                <td class="py-3 text-right">₱{{ number_format($package->price, 2) }}</td>
                                ...
                                <td class="py-3 text-right font-semibold text-green-700">₱{{
                                    number_format($package->total_revenue, 2) }}</td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="6" class="py-8 text-center text-gray-500">No package usage data in this
                                    period</td>
                            </tr>
                            @endforelse
                        </tbody>
                        <tfoot>
                            <tr class="border-t-2 border-gray-300 font-bold bg-green-50">
                                <td colspan="5" class="py-4 text-right text-lg">TOTAL REVENUE:</td>
