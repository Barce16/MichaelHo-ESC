<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800">Revenue Report</h2>
            ...
            <div class="bg-white rounded-lg shadow-sm p-8 print-content">
                ...
                <div class="border-b-2 border-gray-300 pb-6 mb-6">
                    <div class="flex items-start justify-between">
                        ...
                        <div class="text-right">
                            <h2 class="text-xl font-bold text-gray-900">Revenue Report</h2>
                ...
                <div class="grid md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-6 border-2 border-blue-200">
                        <div class="text-sm text-blue-700 font-medium mb-1">Total Payments</div>
                        <div class="text-4xl font-bold text-blue-900">{{ $stats['total_payments'] }}</div>
                    </div>

                    <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-6 border-2 border-purple-200">
                        <div class="text-sm text-purple-700 font-medium mb-2">By Payment Method</div>
                        @foreach($stats['by_method'] as $method => $amount)
                        <div class="flex justify-between text-sm mb-1">
                            <span class="text-purple-800">{{ ucwords(str_replace('_', ' ', $method)) }}:</span>
                            <span class="font-bold text-purple-900">₱{{ number_format($amount, 2) }}</span>
                        </div>
                        @endforeach
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b-2 border-gray-300">
                                ...
                                <th class="text-left py-3 font-semibold text-gray-700">Event</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-gray-200">
                                ...
                                <td class="py-3 font-medium">{{ $payment->billing->event->name ?? '-' }}</td>
                                <td class="py-3">{{ $payment->billing->event->customer->customer_name ?? '-' }}</td>
                                <td class="py-3">
                                    <span
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="py-8 text-center text-gray-500">No payments found in this period
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
