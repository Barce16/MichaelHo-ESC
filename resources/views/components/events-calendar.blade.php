                ...
                <div>
                    ...
                    <div x-show="day.inCurrentMonth"
                        @click="(day.events.length > 0 || day.schedules.length > 0) && showDayModal(day)" :class="{
                            ...
                            'cursor-pointer hover:shadow-md hover:scale-[1.02]': day.events.length > 0 || day.schedules.length > 0,
        ...
        <div @click.stop class="bg-white rounded-xl shadow-2xl max-w-lg w-full max-h-[85vh] overflow-hidden"
            ...
            <div class="overflow-y-auto max-h-[65vh]">
                    ...
                    <div class="space-y-2">
                        <template x-for="event in selectedDayEvents" :key="event.id">
                            <a :href="'{{ $userType === 'admin' ? '/admin/events/' : '/customer/events/' }}' + event.id"
                                class="block p-2.5 rounded-lg bg-violet-50 hover:bg-violet-100 border border-violet-200 transition">
                                <div class="flex items-start justify-between gap-2">
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-[10px] text-violet-500 mt-0.5"
                                            x-text="event.venue || 'Venue TBD'"></p>
                                    </div>
                                    <span
                                        class="px-1.5 py-0.5 rounded-full text-[9px] font-semibold bg-violet-200 text-violet-800 capitalize whitespace-nowrap"
                        ...
                        <template x-for="sched in selectedDaySchedules" :key="sched.id">
                            <a :href="'{{ $userType === 'admin' ? '/admin/events/' : '/customer/events/' }}' + sched.event_id"
                                ...
                                <div class="flex gap-3">
                                    {{-- Large Image --}}
                                    <div class="w-24 h-24 flex-shrink-0 bg-amber-100">
                                    ...
                                    <div class="flex-1 py-2 pr-3 min-w-0">
                                        <div class="flex items-start justify-between gap-2">
                                            <div class="min-w-0">
                                                ...
                                                <p class="text-xs text-amber-600 truncate mt-0.5"
                                                    x-text="sched.event_name"></p>
                                            </div>
                                        ...
                                        {{-- Remarks --}}
                                        <p x-show="sched.remarks" class="text-xs text-amber-700 mt-1.5 line-clamp-2"
                                            x-text="sched.remarks"></p>
                                        {{-- Category badge --}}
                                        <span x-show="sched.category"
                                            class="inline-block mt-1.5 px-1.5 py-0.5 text-[9px] font-medium bg-amber-200/50 text-amber-700 rounded"
                ...
                <div x-show="selectedDayEvents.length === 0 && selectedDaySchedules.length === 0"
                    ...
                    <p class="text-sm">Nothing scheduled for this day</p>
                </div>
            </div>
        </div>
    </div>
